...
int parseDhcpMsgRecv() /* this routine parses dhcp message received */
{
  ...
  /* INICIO DA ALTERACAO PARA ADAPTACAO DA NID ARCHITECTURE */
  
  int sockfd;
  int addr_len;
  int size;
  struct sockaddr_un peer;
  char *str, aux[INET_ADDRSTRLEN];
  
  bzero(&peer, sizeof(peer));
  peer.sun_family = AF_UNIX;
  strcpy(peer.sun_path, "/tmp/dhcp2nidd");
  addr_len = strlen(peer.sun_path) + sizeof(peer.sun_family);

  /* FIM DA ALTERACAO PARA ADAPTACAO DA NID ARCHITECTURE */
...
swend:
  ...
  if ((*(unsigned char *)DhcpOptions.val[53]) == 5){

    //printf("IP = %s\n", inet_ntoa(DhcpMsgRecv->yiaddr));
    inet_ntop(AF_INET, &DhcpMsgRecv->yiaddr, aux, INET_ADDRSTRLEN);
    ...
    if (connect(sockfd, (struct sockaddr *) &peer, addr_len) < 0)
      perror("Connecting");
    
    size = strlen(DhcpOptions.val[17]) + INET_ADDRSTRLEN + 1;
    str = (char *) calloc(size, sizeof(char));
    strcpy(str, DhcpOptions.val[17]);
    strcat(str, aux);
    strcat(str, "|");
